  ...
  return (
    <div className="p-4 space-y-6 bg-[var(--bg-app)] h-full overflow-auto transition-colors duration-300">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-medium text-[var(--text-main)]">Dashboard</h2>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4 max-w-[1600px]">
        {cards.map((card, idx) => (
          <div key={idx} className="bg-[var(--bg-card)] p-4 rounded border border-[var(--border-base)] shadow-sm space-y-3">
            ...
            <div>
              <p className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">{card.label}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-[var(--bg-card)] p-6 rounded border border-[var(--border-base)] shadow-sm">
          <h3 className="text-sm font-bold text-[var(--text-main)] mb-6 uppercase tracking-wider">Recent Activity</h3>
            ...
            {recentSales.map(sale => (
              <div key={sale.id} className="flex items-center gap-4 border-b border-[var(--border-base)] pb-3 last:border-0">
                ...
                <div className="flex-1">
                  <p className="text-sm font-bold text-[var(--text-main)]">New Sale Completed</p>
                  <p className="text-[11px] text-[var(--text-muted)]">Invoice #{sale.invoice_number} • {new Date(sale.created_at).toLocaleTimeString()}</p>
                </div>
                <p className="font-bold text-[var(--text-main)] text-sm">€{sale.grand_total.toFixed(2)}</p>
              </div>
            ))}
        ...
        <div className="bg-[var(--bg-card)] p-6 rounded border border-[var(--border-base)] shadow-sm">
          <h3 className="text-sm font-bold text-[var(--text-main)] mb-6 uppercase tracking-wider">Top Selling Categories</h3>
